...
import numpy as np
import torch
from torch import Tensor, unravel_index
from tqdm import tqdm


@torch.jit.script
def compute_cov(mat: Tensor) -> Tuple[Tensor, Tensor]:
...
@torch.jit.script
def evaluate_features(features: Tensor, sample: Tensor) -> Tensor:
...
def decimation(
    features: Tensor, q: Tensor, feature_threshold: int, num_visibles: int
) -> Tuple[Tensor, Tensor]:
...
def get_random_features(num_features, dataset):
    # Define the region
    min_coords = dataset.min(0)
    max_coords = dataset.max(0)

    # Generate normal vectors
    normals = np.random.randn(num_features, dataset.shape[1])
    normals /= np.linalg.norm(normals, axis=1)[:, np.newaxis]

    # Compute offsets
    offsets = np.minimum(np.dot(normals, min_coords), np.dot(normals, max_coords))
    features = np.hstack([normals, offsets[:, np.newaxis]])
    return features
